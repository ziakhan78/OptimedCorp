...
{
    public class StoreLocatorsController : Controller
    {
        ...
        //{ 
        //    return View(db.StoreLocators.Where(x => x.State == state && x.City == city).ToList());
        // }
        ...
        {
            BindStateFromSL(); 
            return View("Index",db.StoreLocators.Where(x=>x.City==city).ToList());
        }

        [AllowAnonymous]
        [HttpGet]       
        public ActionResult SearchByState(string state)
        {
            BindStateFromSL();
            return View("Index", db.StoreLocators.Where(x => x.State == state).ToList());
        }

        [AllowAnonymous]
        [HttpGet]
        public ActionResult SearchByShop(string shopName)
        {
            BindStateFromSL();

            ViewBag.ShopName = shopName;
            // return View("Index", db.StoreLocators.Where(x => x.ShopName == shopName).ToList());
            return View("Index", db.StoreLocators.Where(x => x.ShopName.Contains(shopName)).ToList());
        ...
        //        var model = db.StoreLocators.Where(x => x.ShopName.Contains(search)).ToList();
        //        return View(model);
        ...
        //        var state1 = db.States.Find(int.Parse(state));
        //        StateName = state1.StateName;

        //        var city1 = db.Cities.Find(int.Parse(city));
        //        CityName = city1.CityName;
        //        ViewBag.StateName = StateName;
        //        ViewBag.CityName = CityName;
        //        var events = db.StoreLocators.Where(x => x.State.Contains(StateName) && x.City.Contains(CityName)).ToList();
        ...
        [AllowAnonymous]
        [HttpGet]
        public ActionResult Index(int? page, string state, string city, string shopName)
        {
            int pageSize = 10;
            ...
            if (state == null && city == null && shopName == null)
            {
                StateName = "Maharashtra";
                CityName = "Mumbai";
                var events = db.StoreLocators.Where(x => x.State.Contains(StateName) && x.City.Contains(CityName)).ToList();
                return View("Index", events.ToPagedList(pageNumber, pageSize));
            }
            ...
            {
                ViewBag.ShopName = shopName;              
                var events = db.StoreLocators.Where(x => x.ShopName.Contains(shopName)).ToList();
                return View("Index", events.ToPagedList(pageNumber, pageSize));
            }

            StateName = "Maharashtra";
            CityName = "Mumbai";
            var events1 = db.StoreLocators.Where(x => x.State.Contains(StateName) && x.City.Contains(CityName)).ToList();
            return View("Index", events1.ToPagedList(pageNumber, pageSize));
        }
        ...
        //    var events = db.StoreLocators.Where(x => x.State == state && x.City == city).ToList();
